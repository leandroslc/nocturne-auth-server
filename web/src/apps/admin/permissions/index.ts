window.searchPermissions = (options) => {
...
window.managePermissions = (options) => {
  const PermissionActionAttribute = 'data-permission-action';

  const searchPermissions = window.searchPermissions(options);
  const { modalId, newPermissionButton, results } = options;
